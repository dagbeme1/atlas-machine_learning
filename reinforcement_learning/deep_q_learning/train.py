    ...
    def train(self):
        ...
        for i, (state, action, reward, next_state, done) in enumerate(batch):
            ...
            if done:
                ...
                # Otherwise, set target to the future discounted reward
                target[action] = reward + self.gamma * \
                    np.amax(next_q_values[i])
            q_values[i] = target  # Update Q-values
        ...
        self.epsilon = max(
            self.epsilon_min,
            self.epsilon *
            self.epsilon_decay)  # Decay epsilon
...
# Initialize agent
input_shape = (4,)  # Input shape for CartPole-v1 environment
num_actions = env.action_space.n  # Number of possible actions
agent = DQNAgent(input_shape, num_actions)  # Creating the DQN agent

# Train agent
train_agent(agent)  # Training the DQN agent

# Save the final policy network
agent.model.save('policy.h5')  # Saving the final policy network
