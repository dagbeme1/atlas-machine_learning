...
def inception_network():
    ...
    layer_1 = K.layers.Conv2D(filters=64, kernel_size=7, padding='same', strides=2, kernel_initializer=initializer, activation='relu')
    output_1 = layer_1(X)

    layer_2 = K.layers.MaxPool2D(pool_size=3, padding='same', strides=2)
    output_2 = layer_2(output_1)

    layer_3R = K.layers.Conv2D(filters=64, kernel_size=1, padding='same', strides=1, kernel_initializer=initializer, activation='relu')
    output_3R = layer_3R(output_2)

    layer_3 = K.layers.Conv2D(filters=192, kernel_size=3, padding='same', strides=1, kernel_initializer=initializer, activation='relu')
    output_3 = layer_3(output_3R)

    layer_4 = K.layers.MaxPool2D(pool_size=3, padding='same', strides=2)
    ...
    layer_7 = K.layers.MaxPool2D(pool_size=3, padding='same', strides=2)
    ...
    layer_13 = K.layers.MaxPool2D(pool_size=3, padding='same', strides=2)
    ...
    layer_16 = K.layers.AvgPool2D(pool_size=7, padding='same', strides=None)
    output_16 = layer_16(output_15)

    layer_17 = K.layers.Dropout(0.4)
    output_17 = layer_17(output_16)

    layer_18 = K.layers.Dense(units=1000, activation='softmax', kernel_initializer=initializer, kernel_regularizer=K.regularizers.l2())
