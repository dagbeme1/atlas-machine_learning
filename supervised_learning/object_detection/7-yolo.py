...
class Yolo:
    ...
    def show_boxes(self, image, boxes, box_classes, box_scores, file_name):
        ...
        for i, box in enumerate(boxes):
            ...
            # Provide the bounding box rectangle settings
            # note: color expressed in BGR coordinates, blue = (255, 0, 0)
            # note: corner coordinates need to be provided as integers
            ...
            # Provide the bounding box text settings
            # note: org is the coordinates of the bottom-left corner
        ...
        # Show the image
        # cv2.imshow does not work on google colab:
        cv2.imshow(file_name, image)
        # cv2_imshow(image)

        # If the s key is pressed, the image should be saved in the
        # directory 'detections', located in the current directory
        key = cv2.waitKey(0)
        if key == ord('s'):
