...
def dropout_forward_prop(X, weights, L, keep_prob):
    ...
    for i in range(L):
        ...
        Zi = np.matmul(weights['W' + str(i + 1)], cache['A' + str(i)]) + weights['b' + str(i + 1)]
        ...
        if i == L - 1:
            ...
            cache['A' + str(i + 1)] = np.exp(Zi) / np.sum(np.exp(Zi), axis=0, keepdims=True)
        else:
            # For other layers, apply the hyperbolic tangent (tanh) activation function
            cache['A' + str(i + 1)] = np.tanh(Zi)

            # Apply dropout by randomly setting a fraction of the values in the layer to zero
            boolean = np.random.rand(cache['A' + str(i + 1)].shape[0], cache['A' + str(i + 1)].shape[1]) < keep_prob
            drop = np.where(boolean == 1, 1, 0)
            cache['A' + str(i + 1)] *= drop
            cache['A' + str(i + 1)] /= keep_prob

            # Store the dropout mask in the cache for later use in backpropagation
